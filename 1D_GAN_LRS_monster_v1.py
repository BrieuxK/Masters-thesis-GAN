...
def build_generator():
    ...
    model.add(Dense(node, input_shape=noise_shape, kernel_initializer=HeNormal(), bias_initializer='zeros'))#, kernel_regularizer=regularizers.l2(0.01))) #initial : 256
...
def train(data_amount, epochs, batch_size, save_interval=2000, callbacks=None):
    ...
    learning_rate = 0.001
    optimizer = SGD(learning_rate, momentum=0.5, nesterov=True, clipvalue=1.0)
    ...
    for epoch in tqdm(range(epochs)):
        ...
        gen_imgs = generator.predict(noise, verbose = 0)                              #verbose here ?
...
#64 nodes 2000, base_lr = 0.0005, max_lr=0.05, mode='triangular2'  : meh
#16 nodes + He initializer, step_size= 2000, base_lr = 0.005, max_lr=0.05, mode='triangular2'
#                                                    = 0.0005 : best so far v3
#16 step_size= 1000, base_lr = 0.00005, max_lr=0.05, mode='triangular2' : good but slightly lower than "best so far"
#8                                                                      : very good but looks slightly smaller than best
#6 layers step_size= 2000, base_lr = 0.00005, max_lr=0.05, mode='triangular2' : best so far v4
#6 layers step_size= 2000, base_lr = 0.00005, max_lr=0.05
#                                                          with bias = 0 : best so far v5
#                                                          batch_size = 600 : best so far v6
...
if batch_size == 2 :
    ...
    # arg = np.squeeze(np.concatenate([resc_stock_np,resc_stock_np]))
    arg2 = np.concatenate([selected_simu_np, selected_simu_np, selected_simu_np])#, selected_simu_np, selected_simu_np])
