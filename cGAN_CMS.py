...
def cms_cut(arr, pt_cut=None, invM_cut=None, met_cut=None):
    # Initialize a mask of all True values
    mask = np.ones(len(arr), dtype=bool)
    
    # Apply pt_cut if provided
    if pt_cut is not None:
        pt_min, pt_max = pt_cut
        mask &= (arr[:, 0] >= pt_min) & (arr[:, 0] <= pt_max)
    
    # Apply invM_cut if provided
    if invM_cut is not None:
        invM_min, invM_max = invM_cut
        mask &= (arr[:, 1] >= invM_min) & (arr[:, 1] <= invM_max)
    
    # Apply met_cut if provided
    if met_cut is not None:
        met_min, met_max = met_cut
        mask &= (arr[:, 2] >= met_min) & (arr[:, 2] <= met_max)
...
numpy_array[:, [0, 2]] = numpy_array[:, [2, 0]]
...
sample_new = cms_cut(sample, pt_cut, invM_cut, met_cut)
...
# plt.hist(sample_new[:, 0], bins=75, color='blue', label='Refined')
# plt.hist(sample_new_v2[:, 0], bins=75, edgecolor='red', linewidth=3, histtype='step', label='Raw')
# plt.title("Leading muon transverse momentum")
# plt.xlabel("GeV")
# plt.ylabel("Probability")
# plt.legend()
# plt.show()

# plt.hist(sample_new[:, 1], bins=75, color='blue', label='Refined')
# plt.hist(sample_new_v2[:, 1], bins=75, edgecolor='red', linewidth=3, histtype='step', label='Raw')
...
# plt.hist(sample_new[:, 2], bins=75, color='blue', label='Refined')
# plt.hist(sample_new_v2[:, 2], bins=75, edgecolor='red', linewidth=3, histtype='step', label='Raw')
...
# plt.hist(sample_new[:, 3], bins= [0,1,2,3,4,5], color='blue', label='Refined')
# plt.hist(sample_new_v2[:, 3],bins= [0,1,2,3,4,5], edgecolor='red', linewidth=3, histtype='step', label='Raw')
...
observables = sample_new[:, :-1]
label_np = sample_new[:, -1]
...
def train(epochs, batch_size=128):
    ...
    X_train_raw, x_test, y_train, y_test = train_test_split(observables, y, test_size=0.1)
...
selected_simu_np = observables[:,0]
selected_simu_np1 = observables[:,1]
selected_simu_np2 = observables[:,2]
...
arg2 = selected_simu_np

how_many = len(observables)//epochs
print(how_many)

arg = np.tile(np.squeeze(resc_stock_safe[:,0]), how_many)
...
arg21 = selected_simu_np1
arg1 = np.tile(np.squeeze(resc_stock_safe[:,1]), how_many)
...
arg211 = selected_simu_np2
arg11 = np.tile(np.squeeze(resc_stock_safe[:,2]), how_many)
